          ...
          <div className="flex items-end justify-between">
            <div>
              {treatment.accredited && (
                <span className="mb-2 inline-flex items-center gap-1 rounded-full bg-primary/90 px-3 py-1 text-xs font-medium text-primary-foreground">
          ...
          <div className="flex flex-col gap-8 lg:flex-row">
              ...
              <div className="mt-8">
                  ...
                  <div>
                    ...
                    <div className="mt-8 grid gap-4 sm:grid-cols-2">
                      <div className="rounded-xl bg-muted/50 p-5">
                        ...
                        <p className="mt-1 font-medium">{treatment.specialty}</p>
                      </div>
                      <div className="rounded-xl bg-muted/50 p-5">
                        <p className="text-sm text-muted-foreground">Duration</p>
                        <p className="mt-1 font-medium">{treatment.duration}</p>
                      </div>
                      <div className="rounded-xl bg-muted/50 p-5">
                        <p className="text-sm text-muted-foreground">Location</p>
                        <p className="mt-1 font-medium">{treatment.city}, {treatment.country}</p>
                      </div>
                      <div className="rounded-xl bg-muted/50 p-5">
                        <p className="text-sm text-muted-foreground">Rating</p>
                        <p className="mt-1 flex items-center gap-1 font-medium"><Star className="h-4 w-4 fill-current text-accent" />{treatment.rating} ({treatment.reviewCount} reviews)</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === "Provider" && (
                  <div>
                    ...
                    <p className="mt-4 leading-relaxed text-muted-foreground">
                      {treatment.provider} is a leading healthcare facility located in {treatment.city}, {treatment.country}. 
                      With state-of-the-art equipment and internationally trained medical professionals, the centre has served 
                      over 5,000 international patients with a {treatment.rating}★ satisfaction rating.
                    </p>
                    <div className="mt-6 rounded-xl border border-border p-5">
                      <div className="flex items-center gap-4">
                        {treatment.accredited && <BadgeCheck className="h-6 w-6 text-primary" />}
                        <div>
                          <p className="font-medium">JCI Accredited</p>
                          <p className="text-sm text-muted-foreground">Internationally recognised healthcare quality certification</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === "Location" && (
                  <div>
                    ...
                    <p className="mt-4 text-muted-foreground">{treatment.city}, {treatment.country}</p>
            ...
            <div className="w-full lg:w-80">
              <div className="sticky top-24 rounded-xl border border-border bg-card p-6 shadow-lg">
                ...
                <p className="mt-1 font-serif text-3xl font-bold text-primary">{treatment.currency}{treatment.price.toLocaleString()}</p>
                ...
                <div className="mt-6 rounded-lg border border-border p-4">
                  ...
                  <p className="mt-1 text-sm">
                    Pay in <span className="font-semibold">4 instalments</span> of {treatment.currency}{Math.round(treatment.price / 4).toLocaleString()}/month
                  </p>
