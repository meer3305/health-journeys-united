    ...
    return (
      <div className="flex min-h-[80vh] items-center justify-center">
        <div className="max-w-md text-center">
          ...
          <Button className="mt-8 bg-accent text-accent-foreground" onClick={() => navigate("/dashboard")}>Go to Dashboard</Button>
        </div>
      </div>
    );
  }
      ...
      <Container>
        <div className="mx-auto max-w-xl">
          <div className="mb-10 flex items-center justify-center gap-4">
            <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium ${step >= 1 ? "bg-accent text-accent-foreground" : "bg-muted text-muted-foreground"}`}>1</div>
            <div className={`h-0.5 w-16 ${step >= 2 ? "bg-accent" : "bg-border"}`} />
            <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium ${step >= 2 ? "bg-accent text-accent-foreground" : "bg-muted text-muted-foreground"}`}>2</div>
          </div>
          ...
          {step === 1 && (
            <div>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2">
                <button onClick={() => { setPath("medical"); setStep(2); }}
                  className="flex flex-col items-center rounded-xl border-2 p-8 text-center transition-all hover:border-primary hover:shadow-md border-border">
                ...
                <button onClick={() => { setPath("wellness"); setStep(2); }}
                  className="flex flex-col items-center rounded-xl border-2 p-8 text-center transition-all hover:border-primary hover:shadow-md border-border">
            ...
            <div>
              ...
              <form className="mt-10 space-y-5" onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }}>
                <div className="grid gap-5 sm:grid-cols-2">
                  <div><label className="mb-1.5 block text-sm font-medium">Full Name</label><input type="text" required className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="James Wilson" /></div>
                  <div><label className="mb-1.5 block text-sm font-medium">Email</label><input type="email" required className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="james@example.com" /></div>
                </div>
                <div className="grid gap-5 sm:grid-cols-2">
                  <div><label className="mb-1.5 block text-sm font-medium">Phone</label><input type="tel" className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="+44 7700 900000" /></div>
                  <div><label className="mb-1.5 block text-sm font-medium">Country</label><input type="text" className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="United Kingdom" /></div>
                </div>
                <div>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <Button type="submit" className="flex-1 bg-accent text-accent-foreground hover:bg-accent/90">Submit</Button>
                </div>
              </form>
            </div>
          )}
