...
import { bookings as mockBookings, type Booking } from "@/data/dashboardData";
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center">
          ...
          <p className="mt-4 font-medium text-muted-foreground">No bookings found</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((b, i) => {
            ...
            return (
              ...
              >
                <img src={b.image} alt={b.treatment} className="h-20 w-20 sm:h-24 sm:w-24 rounded-xl object-cover shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h4 className="font-serif text-base font-semibold">{b.treatment}</h4>
                ...
                <div className="text-right shrink-0">
                  <p className="font-serif text-xl font-bold text-primary">{b.currency}{b.price.toLocaleString()}</p>
